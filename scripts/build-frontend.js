...
const PUBLIC_FILES = [
  ...
  'app.js', 'privacy.js', 'legal.js', 'style.css'
];

async function copyPublicFile(output, file, transformHtml) {
  let data = file.endsWith('.png')
    ? await readFile(new URL('../public/' + file, import.meta.url))
    : await readFile(new URL('../public/' + file, import.meta.url), 'utf8');
  if (file.endsWith('.html')) data = transformHtml(data, file);
...
export async function buildFrontend(env = process.env) {
  ...
  for (const file of PUBLIC_FILES) {
    await copyPublicFile(output, file, data => data.replace(/(<meta charset="[^"]+">)/i, `$1<meta http-equiv="Content-Security-Policy" content="${csp}">`));
  }
...
export async function buildSupabaseFrontend(env = process.env) {
  ...
  for (const file of PUBLIC_FILES) {
    await copyPublicFile(output, file, (data, filename) => {
      let content = data.replace(/(<meta charset="[^"]+">)/i, `$1<meta http-equiv="Content-Security-Policy" content="${csp}">`);
      if (filename === 'index.html') content = content.replace('<script src="/app.js"', '<script src="/transport.js" defer></script><script src="/app.js"');
      return content;
    });
  }
