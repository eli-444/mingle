...
function metrics(data) {
  ...
  return { ...data, daily, monthly: data.monthly.map(row => ({ ...row, month: row.month.slice(0, 7) })), today: daily.find(row => row.date === new Date().toISOString().slice(0, 10)) || { pageviews: 0, peak: 0, reports: 0 } };
}
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (path === 'visit' && req.method === 'POST') {
        await rate('visit:' + ip, 60, 60);
        const country = env.VERCEL && /^[A-Z]{2}$/.test(req.headers['x-vercel-ip-country'] || '') ? req.headers['x-vercel-ip-country'] : null;
        await rpc('count_pageview', { p_country: country });
        return json(200, { ok: true });
      }
      if (path === 'contact' && req.method === 'POST') {
        await rate('contact:' + ip, 5, 3600);
        const input = await readBody(req);
        if (typeof input.firstName !== 'string' || input.firstName.trim().length < 1 || input.firstName.length > 100 ||
            typeof input.lastName !== 'string' || input.lastName.trim().length < 1 || input.lastName.length > 100 ||
            typeof input.email !== 'string' || input.email.length > 320 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email.trim()) ||
            typeof input.message !== 'string' || input.message.trim().length < 1 || input.message.length > 5000) throw fail(400, 'Invalid contact request.');
        await rpc('contact_create', { p_first_name: input.firstName, p_last_name: input.lastName, p_email: input.email, p_message: input.message });
        return json(200, { ok: true });
      }
      ...
      if (isAdmin) {
        ...
        if (endpoint === 'dashboard' && req.method === 'GET') {
          ...
          data.contacts = data.contacts.map(r => ({ id: r.id, firstName: r.first_name, lastName: r.last_name, email: r.email, message: r.message, status: r.status, notes: r.notes, created: r.created_at }));
        ...
        if (endpoint === 'logout') {
          ...
          if (!uuid(input.id)) throw fail(400, 'Invalid contact request.');
          if (input.action === 'delete') await rpc('contact_delete', { p_request: input.id, p_actor });
          else if (input.action === 'update' && ['new', 'in_progress', 'closed'].includes(input.status) && typeof input.notes === 'string' && input.notes.length <= 2000) await rpc('contact_update', { p_request: input.id, p_status: input.status, p_notes: input.notes, p_actor });
          else throw fail(400, 'Invalid contact request.');
