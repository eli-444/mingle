...
function loggedIn(value) { $('login').hidden = value; $('dashboard').hidden = $('logout').hidden = !value; }
...
async function load() {
  ...
  $('live').replaceChildren(...[['Connectés', data.online.connected], ['En attente', data.online.waiting], ['Duos actifs', data.online.conversations]].map(([label, number]) => { const div = el('div', undefined, 'card'); div.append(el('strong', number), el('span', label)); return div; }));
  rows('monthly', data.monthly, ['month', 'connections', 'visitors', 'matches', 'reports']); rows('daily', data.daily, ['date', 'connections', 'matches', 'reports']);
  ...
  for (const key of ['operator', 'contact', 'hosting']) $(key).value = data.policy[key]; $('retention').value = data.policy.retentionDays;
...
$('settings').onsubmit = async e => { e.preventDefault(); try { await api('settings', { operator: $('operator').value.trim(), contact: $('contact').value.trim(), hosting: $('hosting').value.trim(), retentionDays: Number($('retention').value) }); await load(); notice('Informations de confidentialité enregistrées.'); } catch (e) { notice(e.message); } };
